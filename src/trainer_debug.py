...
class Trainer:
    ...
    def __init__(
            ...
            test_dataset,
            config,
            wandb_project: str = "spiking_lm",
            wandb_prefix: str = "spikeGPT",
        ):
        ...
        self.dev_loss = -1
        self.steps = 0
        self.wandb_logging = self.config.wandb_logging and 'wandb' in sys.modules
        ...
        if self.wandb_logging:
            ...
            wandb.init(
                ...
                name=f'{self.wandb_run_prefix}-{self.get_run_name()}-{curr_time}',
                config=cfg,
                save_code=False,
            )
    ...
    def train(self):
        ...
        for epoch in range(config.max_epochs):
            ...
            self.run_epoch('train', model=model, optimizer=optimizer, config=config, epoch=epoch)
            if config.validate:
                self.run_epoch('valid', model=model, optimizer=optimizer, config=config, epoch=epoch)
            if config.test:
                self.run_epoch('test', model=model, optimizer=optimizer, config=config, epoch=epoch)

            # save with a given frequency or if it's the last epoch
            if (
