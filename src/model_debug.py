...
CPP_EXT_VERBOSE = os.environ.get('CPP_EXT_VERBOSE')
...
class L2Wrap(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        """
        what it does:
            - replaces nonmax elements at dim -1 with 0
            - multiplies max elements with a factor
        """
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.head = nn.Linear(config.n_embd, config.vocab_size, bias=False) # why don't we need a bias
    ...
    def configure_optimizers(self, train_config):
        ...
        try:
            ...
            print(f'DeepSpeed not found, using torch optimizer instead of FusedAdam')
