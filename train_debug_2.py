...
class Trainer:
    ...
    def run_epoch(self, split, optimizer, model, epoch):
    ...
    def train(self):
        ...
        for epoch in range(config.max_epochs):
            ...
            self.run_epoch('train', optimizer, model, epoch)
            ...
            for eval_mode in eval_modes:
                self.run_epoch(eval_mode, optimizer, model, epoch)
                if self.wandb_logging_enabled():
                    wandb.log(
                        {
...
def main():
    ...
    parser.add_argument('-c', '--config', type=str, default='config/conf_NEW_test_0.yml')
    ...
    train_dataset = load_from_disk(args.pretokenized_path)
    train_dataset.__getitems__ = None
    valid_dataset = None
    # valid_dataset.__getitems__ = None
    test_dataset = None
    # test_dataset.__getitems__ = None

    vocab_size = tokenizer.get_vocab_size()


    # won't this take a VERY long time???
    input_ids = train_dataset.data.column("input_ids")
    train_lengths = np.vectorize(len)(np.array(input_ids, dtype=object))

    train_packing_sampler = SamplePackingBatchSampler(
        batch_size=args.batch_size,
        ctx_len=args.ctx_len,
        group_size=args.packing_group_size,
        lengths=train_lengths,
        sampler=RandomSampler(train_dataset), # accelerate will synchronize rng states across shards
    )
